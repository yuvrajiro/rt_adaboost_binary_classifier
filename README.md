# Adaboost Binary Classifier Implementation

## Project Description

This repository is a dockerized implementation of the Adaboost classifier. 
It is implemented in flexible way that it can be used with any binary classification dataset with the use of JSON-formatted data schema file.
...
- Implementation contains a full-suite of functionality such as:
  ...
  - A Adaboost Classifier algorithm built using **Scikit-Learn**
...
- **`src/`**: This directory holds the source code for the project. It is further divided into various subdirectories :
  ...
  - **`prediction/`**: for prediction model (Adaboost Classifier) script
...
- Run the script `src/train.py` to train the  Adaboost Classifier model. This 
  will save the model artifacts, including the preprocessing pipeline and label encoder, in the path `./model_inputs_outputs/model/artifacts/`. If you want to run with hyperparameter tuning then include the `-t` flag. This will also save the hyperparameter tuning results in the path `./model_inputs_outputs/outputs/hpt_outputs/`.
